...
public class TaskDaoImpl implements TaskDao {
	// 存放任务信息
	private volatile static ConcurrentMap<String, CommandTasker> map = null;

	public TaskDaoImpl(int size) {
		if (null==TaskDaoImpl.map) {
			synchronized (TaskDaoImpl.class) {
				if (null==TaskDaoImpl.map) {
					TaskDaoImpl.map = new ConcurrentHashMap<>(size);
				}
			}
		}
	}
	...
	public boolean isHave(String id) {
		System.out.println("isHave:"+id+":"+map.containsKey(id));
